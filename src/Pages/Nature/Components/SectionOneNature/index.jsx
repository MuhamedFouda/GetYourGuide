...
export default function SectionOneNature() {
    const [VeiwWishlist, setVeiwWisglist] = useRecoilState($Wishlist)
    const [CardView, setCardView] = useRecoilState($CardView)
    const navigate = useNavigate();
    const [cards, setcard] = useState([])
    axios.get("http://localhost:3000/NatureSectionOne").then((ele) => {
        setcard(ele.data)
    })
    return (
        <div className='sectionOneNature wow animate__animated animate__zoomInDown'>
