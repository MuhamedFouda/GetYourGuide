...
export default function SectionOneSports() {
    const [VeiwWishlist, setVeiwWisglist] = useRecoilState($Wishlist)
    const [CardView, setCardView] = useRecoilState($CardView)
    const navigate = useNavigate(); const [cards, setcard] = useState([])
    axios.get("http://localhost:3000/SportsSectionOne").then((ele) => {
        setcard(ele.data)
    })
    return (
        <div className='sectionOneSports wow animate__animated animate__zoomInDown'>
